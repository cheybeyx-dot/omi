...
type CheckoutStep =
  ...
  | "pending_crypto"
  | "declined";
type PayMethod = "card" | "korapay" | "trustwallet";
...
const getPaymentMethodsForCountry = (
  ...
  if (amount > 150) {
    methods.push("trustwallet");
    methods.push("card");
    return methods;
  }
  methods.push("trustwallet");
  methods.push("card");
  if (KORAPAY_COUNTRIES.has(countryCode)) methods.push("korapay");
  return methods;
};
...
function CheckoutInner() {
  ...
  const isKoraPay = KORAPAY_COUNTRIES.has(countryCode);
  const effectivePrice = payMethod === "trustwallet" ? discountedPrice : price;
  ...
  useEffect(() => {
    ...
    setPayMethod(isKoraPay ? "korapay" : "card");
  }, [countryCode, isKoraPay]);
  ...
  const receiptData = {
    ...
    discounted: payMethod === "trustwallet",
    originalAmount: price,
    walletAddress: payMethod === "trustwallet" ? twSenderAddress : undefined,
  };
              ...
              {(() => {
                ...
                return (
                  <div className="space-y-3 mb-6">
                    <button
                      onClick={() => setPayMethod("trustwallet")}
                      className={`w-full p-4 rounded-xl transition-all border-2 relative overflow-hidden ${payMethod === "trustwallet" ? "bg-gradient-to-r from-violet-600/40 to-purple-600/40 border-violet-400" : "bg-slate-800/50 border-slate-600 hover:border-violet-400/50"}`}
                    >
                    ...
                    {availableMethods.includes("card") && (
                      <button
                        ...
                        className={`w-full p-4 rounded-xl transition-all border-2 ${payMethod === "card" ? "bg-slate-700/50 border-slate-400" : "bg-slate-800/30 border-slate-700 hover:border-slate-500"}`}
                      >
                        <div className="flex items-center gap-3">
                    ...
                    {availableMethods.includes("korapay") && (
                      <button
                        onClick={() => setPayMethod("korapay")}
                        className={`w-full p-4 rounded-xl transition-all border-2 ${payMethod === "korapay" ? "bg-blue-700/20 border-blue-400" : "bg-slate-800/30 border-slate-700 hover:border-blue-500/50"}`}
                      >
                        <div className="flex items-center gap-3">
                          <div className="text-xl">🏦</div>
                          <div className="text-left flex-1">
              ...
              <form onSubmit={handleSubmit}>
                {payMethod === "card" && (
                  ...
                  >
                    <div className="mb-4">
                      ...
                      <div className="relative">
                        <input
                          ...
                          className={`w-full px-4 py-3 bg-black/30 rounded-lg text-white placeholder-slate-500 font-mono focus:outline-none transition-all ${cardType === "unsupported" && card.number ? "border-2 border-red-500" : "border border-slate-700 focus:border-slate-500"}`}
                        />
                    ...
                    {cardPaymentRequiresOtp && (
                      <div className="mb-4 p-4 rounded-lg bg-blue-500/10 border border-blue-500/30">
                        <p className="text-blue-200 text-xs mb-4 font-semibold">
                          🔐 Your bank is sending an OTP verification code to
                          the phone number linked to your card
                        </p>
                        {!otpSent ? (
                          <button
                            type="button"
                            onClick={() => {
                              if (!cardPhoneNumber.replace(/\D/g, "").length) {
                                alert("Please enter a valid phone number");
                                return;
                              }
                              if (!isValidCard(card)) {
                                alert("Please enter valid card details first");
                                return;
                              }
                              setOtpSent(true);
                              setOtp("");
                            ...
                            className="w-full bg-blue-600 hover:bg-blue-500 text-white text-sm font-bold py-2 rounded-lg transition-all"
                          >
                            Send OTP to {cardPhoneNumber || "****"}
                          </button>
                        ) : (
                          <>
                            <div className="mb-3">
                              <label className="block text-blue-200 text-xs font-semibold mb-2">
                                Enter the OTP code sent to {cardPhoneNumber}
                              </label>
                              <input
                                type="text"
                                placeholder="000000"
                                ...
                                maxLength={6}
                                className={`w-full px-3 py-2 bg-black/30 rounded-lg text-white text-center tracking-widest font-bold focus:outline-none ${otpError ? "border-2 border-red-500" : "border border-blue-500/30"}`}
                              />
                              {otpError && (
                            ...
                            <button
                              ...
                              className="w-full bg-emerald-600 hover:bg-emerald-500 text-white text-sm font-bold py-2 rounded-lg mb-3"
                            >
                              Verify OTP
                            </button>
                            <div className="space-y-2">
                              <button
                                ...
                                className="w-full bg-slate-700 hover:bg-slate-600 text-white text-xs font-bold py-2 rounded-lg"
                              >
                                Return to Dashboard
                              </button>
                              <button
                                ...
                                onClick={() => {
                                  ...
                                  setOtpError("");
                                  setOtpAttempts(0);
                                  setPayMethod("trustwallet");
                                }}
                                className="w-full bg-slate-700 hover:bg-slate-600 text-white text-xs font-bold py-2 rounded-lg"
                              >
                                Change to Crypto Payment
                              </button>
                            </div>
                          </>
                        )}
                    ...
                    {!otpSent ? (
                      <button
                        ...
                        onClick={() => {
                          if (cardType === "unsupported") {
                            alert("Only Visa and Mastercard are accepted");
                            return;
                          }
                          if (!isValidCard(card)) {
                        ...
                        disabled={
                          ...
                          !isValidCard(card) ||
                          !cardPhoneNumber.replace(/\D/g, "").length
                        }
                      ...
                      >
                        <Lock size={16} /> Continue to Verification
                      </button>
                    ) : (
                  ...
                  >
                    <div className="mb-4 p-3 bg-blue-500/10 border border-blue-500/30 rounded-lg">
                      <p className="text-blue-200 text-xs">
                        💬 Enter your phone number to proceed to KoraPay
                      </p>
                    </div>
                    <div className="mb-4">
                      <label className="block text-white text-sm font-bold mb-2">
                    ...
                    >
                      ...
                      {kpLoading ? "Redirecting..." : "Proceed to KoraPay"}
                    </button>
                  </div>
                )}
                {payMethod === "trustwallet" && (
                  ...
                  >
                    <div>
                      ...
                      <p className="text-slate-500 text-xs">
                        Open TrustWallet → scan QR or copy address → send exact
                        amount
                      </p>
                    </div>
                    {!configLoaded ? (
