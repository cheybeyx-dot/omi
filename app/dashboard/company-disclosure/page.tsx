...
const CO = {
  ...
  reg: "Registration No. OT-2024-GB-7741902",
  address:
    "Level 14, One Canada Square, Canary Wharf, London E14 5AB, United Kingdom",
      ...
      content: (
        <DocPage>
          ...
          <P>
            ...
            Kingdom, with regional operations management teams located in Lagos,
            Singapore, Dubai, Toronto, and Zurich. All financial operations,
            compliance oversight, and participant relations are coordinated from
            the London headquarters under the supervision of the Board of
            Directors, chaired by Dmitriy Ardalio.
          </P>
      ...
      content: (
        <DocPage>
          <PageHeader
            ...
            title="How GPU Computing Generates Revenue"
            sub="Detailed explanation of the OmniTask Pro revenue model and economic infrastructure"
          />

          <H2>2.1 The Fundamental Economics of GPU Computation</H2>
          <P>
            Graphics Processing Units — originally designed for video rendering
            — have become the primary computing architecture for artificial
            intelligence model training, inference, data transformation, and
            scientific computation. The AI industry alone spent in excess of $91
            billion on GPU computing infrastructure in 2025, a figure projected
            to exceed $200 billion by 2028. This extraordinary demand creates a
            commercial opportunity for operators who can supply verified,
            high-performance GPU capacity to enterprise buyers at competitive
            rates below the pricing of hyperscale cloud providers.
          </P>
          <P>
            OmniTask Pro's platform aggregates GPU capacity from thousands of
            individual node operators worldwide, presents this capacity as a
            unified enterprise-grade computing resource, and sells access to
            enterprises, corporations, research institutions, and technology
            companies. Revenue generated from enterprise buyers is distributed
            to node operators according to verified computational contribution,
            with OmniTask Pro retaining a platform fee to cover infrastructure,
            compliance, and operational costs. Node operators receive 73% of
            gross compute revenue attributable to their hardware allocation.
          </P>

          <H2>2.2 Primary Revenue Streams</H2>
          <H3>2.2.1 GPU Compute Hour Leasing</H3>
          <P>
            The primary revenue mechanism is the leasing of verified GPU compute
            hours to enterprise clients under contracted agreements ranging from
            30-day rolling commitments to 24-month fixed-price contracts.
            OmniTask Pro GPUs processed 14.7 million compute hours in Q1 2026,
            with average billing rates from $2.10 per hour (entry-tier hardware)
            to $18.40 per hour (premium H100 cluster configurations).
          </P>

          <H3>2.2.2 AI Model Training Contracts</H3>
          <P>
            Large language model training and fine-tuning engagements represent
            the highest-value category. A single LLM training run may consume
            50,000 to 500,000 GPU hours over two to twelve weeks. These
            contracts are negotiated with AI laboratories at rates of $4.50 to
            $22.00 per GPU hour depending on hardware specification, exclusivity
            requirements, and throughput guarantees.
          </P>

          <H3>2.2.3 Inference Serving & Validation Tasks</H3>
          <P>
            Beyond training, AI companies require continuous GPU capacity for
            inference. On the OmniTask Pro platform, this manifests as RLHF
            (Reinforcement Learning from Human Feedback) validation tasks and
            neural output verification tasks, visible in the Tasks section of
            the dashboard. Task compensation ranges from $0.08 to $0.65 per
            completed validation, with experienced operators completing 80–400
            tasks per day.
          </P>

          <H3>2.2.4 Thermal Calibration & Diagnostic Processing</H3>
          <P>
            Enterprise clients whose sensitive computations require consistent
            hardware performance pay a premium for GPU nodes that have completed
            recent calibration protocols. Operators completing these tasks
            receive $0.30 to $0.55 per calibration cycle, available
            approximately once per 24-hour cycle through the Tasks dashboard.
          </P>

          <H2>2.3 OmniTask Pro Platform Earnings Structure</H2>
          <div className="border border-slate-300 my-5 text-sm">
            <div className="bg-slate-800 text-white px-5 py-3 grid grid-cols-4 gap-4 text-xs font-bold uppercase tracking-wider">
            ...
            {[
              ["Flexible (rolling)", "Variable", "Variable", "0.13% / day"],
              [
                "6-Month Contract",
                "52% of capital",
                "93% of capital",
              ...
              <div
                key={t}
                className="px-5 py-3 grid grid-cols-4 gap-4 border-t border-slate-200"
              >
                <span className="font-semibold text-slate-800">{t}</span>
                <span className="text-slate-700">{mn}</span>
                <span className="text-slate-700">{mx}</span>
                <span className="text-slate-500 text-xs">{b}</span>
              </div>
            ))}
          </div>
          <Alert type="warn">
            <strong>Risk Disclosure — Section 2.3:</strong> Projected return
            figures are estimates derived from historical platform performance
            and current enterprise contract rates. They do not constitute a
            guarantee of earnings. Actual returns may be materially lower or
            higher. Capital allocated to contract-based tiers is subject to full
            lock-up for the stated term.
          </Alert>
