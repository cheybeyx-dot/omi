...
function KeyGenerator({ onKeyValidated }: { onKeyValidated: (key: string) => void }) {
  // History is local-only for display — source of truth is the DB
  const [history,      setHistory]      = useState<KeyRecord[]>([]);
  const [activeRecord, setActiveRecord] = useState<KeyRecord | null>(null);
  ...
  // Countdown timer for the active key
  useEffect(() => {
    if (timerRef.current) clearInterval(timerRef.current);
    if (!activeRecord) return;

    timerRef.current = setInterval(() => {
      const remaining = activeRecord.expiresAt - Date.now();
      if (remaining <= 0) {
        setTimeLeft(0);
        setActiveRecord(null);
        setHistory((prev) =>
          prev.map((k) => k.key === activeRecord.key ? { ...k, status: "expired" } : k)
        );
  ...
  const generate = useCallback(async () => {
    ...
    // Optimistically mark previous active as expired in local history
    setHistory((prev) =>
      prev.map((k) => k.status === "active" ? { ...k, status: "expired" } : k)
    );
    ...
    try {
      ...
      if (!session) {
        setGenError("Not logged in. Please sign in first.");
        setGenerating(false);
        return;
      }
      ...
      if (!res.ok) {
        setGenError(data.error || "Failed to generate key. Please try again.");
        return;
      }

      // data = { key, expiresAt (ISO string), emailSent }
      ...
      setTimeLeft(record.expiresAt - Date.now());
      setEmailSent(data.emailSent === true);

      if (data.emailSent === false && data.warning) {
        setGenError(data.warning);
      }
    } catch (err: any) {
      setGenError(err.message || "Network error. Check your connection and try again.");
    } finally {
      setGenerating(false);
    }
  }, []);
  ...
  function handleCopy() {
    if (!activeRecord) return;
    navigator.clipboard.writeText(activeRecord.key).catch(() => {});
    setCopied(true);
  ...
  async function validateInput() {
    ...
    if (!trimmed) { setInputError("Please paste your key first."); return; }
    ...
    try {
      ...
      if (!session) {
        setInputError("Not logged in. Please sign in first.");
        setValidating(false);
        return;
      }

      const res = await fetch("/api/license-key/validate", {
        method:  "POST",
        headers: { 
          "Content-Type": "application/json",
          "Authorization": `Bearer ${session.access_token}`,
      ...
      if (!res.ok) {
        setInputError(data.error || "Validation failed. Please try again.");
        return;
      }

      // Mark in local history
      setHistory((prev) =>
        prev.map((k) => k.key === trimmed ? { ...k, status: "used" } : k)
      );
      setActiveRecord(null);
      setInputOk(true);
      onKeyValidated(trimmed);
    } catch (err: any) {
      setInputError(err.message || "Network error. Check your connection and try again.");
    } finally {
      setValidating(false);
    }
  }

  const urgentColor = timeLeft > 0 && timeLeft < 120_000 ? "text-red-500" : "text-amber-600";
    ...
    <div className="space-y-8" style={{ fontFamily: "system-ui, sans-serif" }}>
      ...
      <div className="bg-gray-50 border border-gray-200 rounded-xl p-4 space-y-2">
        <p className="text-xs font-bold text-gray-700 flex items-center gap-1.5">
          <Key size={11} /> How the license key system works
        </p>
        <p className="text-xs text-gray-500 leading-relaxed">
          When you click <strong className="text-gray-700">Generate License Key</strong>, a unique key is created
          in our database and emailed to your registered address. The key is valid for{" "}
          <strong className="text-gray-700">15 minutes</strong> and can only be used{" "}
          <strong className="text-gray-700">once</strong>. Copy it from your email (or from this page),
          paste it in Step B, and click <strong className="text-gray-700">Validate</strong>.
          Only a validated key unlocks the checkout button.
        </p>
      </div>
      ...
      <div>
        ...
        <p className="text-xs text-gray-500 leading-relaxed mb-4">
          Click the button below. A unique key will be saved to your account and{" "}
          <strong className="text-gray-700">emailed to your registered address</strong> immediately.
          You can also copy it directly from this page. Generating a new key cancels any previous one.
        </p>
        ...
        <div className="flex items-center gap-3 flex-wrap">
          <button
            onClick={generate}
            disabled={generating}
            className="flex items-center gap-2 px-5 py-2.5 rounded-lg bg-black text-white text-sm font-semibold hover:bg-gray-800 transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
          >
            {generating
              ? <><Loader2 size={13} className="animate-spin" /> Generating…</>
              : <><Zap size={13} /> Generate License Key</>}
          </button>

          {activeRecord && !generating && (
            <p className={`text-xs font-semibold flex items-center gap-1.5 ${urgentColor}`}>
              <Timer size={12} /> Expires in {fmt(timeLeft)}
            </p>
          )}
        </div>
        ...
        {/* Error */}
        {genError && (
          <p className="text-xs text-amber-600 mt-3 flex items-start gap-1.5 bg-amber-50 border border-amber-100 rounded-lg px-3 py-2.5">
            <AlertCircle size={11} className="mt-0.5 shrink-0" /> {genError}
          </p>
        )}
        ...
        {emailSent && !genError && (
          <div className="mt-3 flex items-start gap-2 bg-green-50 border border-green-200 rounded-lg px-3 py-2.5">
            <Mail size={12} className="text-green-600 mt-0.5 shrink-0" />
            <p className="text-xs text-green-700 leading-relaxed">
              <strong>Key emailed.</strong> Check your inbox — the key has been sent to your registered email address.
              You can also copy it directly from this page below.
            </p>
          </div>
        )}
        ...
        {activeRecord && (
          <div className="mt-4 space-y-1">
            <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">Your current key</p>
            <div className="flex items-center gap-3">
              <div className="flex-1 border border-gray-200 rounded-lg px-4 py-3 font-mono text-base font-bold text-gray-800 tracking-widest bg-gray-50 select-all">
                {activeRecord.key}
              </div>
              <button
                onClick={handleCopy}
                className="flex items-center gap-1.5 border border-gray-200 hover:border-gray-400 rounded-lg px-4 py-3 text-sm font-semibold text-gray-600 hover:text-black transition-all shrink-0"
              >
        ...
        {!activeRecord && history.length > 0 && (
          <p className="text-xs text-red-500 mt-3 flex items-center gap-1.5">
            <XCircle size={12} /> No active key. Generate a new one above.
          </p>
        )}
      </div>

      {/* ── STEP B: PASTE & VALIDATE ── */}
      <div>
        <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-gray-400 mb-2">
          Step B — Paste &amp; validate your key
        </p>
        <p className="text-xs text-gray-500 leading-relaxed mb-4">
          Copy the key from your email or from Step A above, paste it exactly as shown below, then click{" "}
          <strong className="text-gray-700">Validate Key</strong>. The key must be validated within 15 minutes
          of generation. You cannot proceed to checkout without a validated key.
        </p>
        ...
        <div className="flex gap-3">
          <input
            type="text"
            value={inputKey}
            onChange={(e) => { setInputKey(e.target.value); setInputError(""); setInputOk(false); }}
            placeholder="OMNI-XXXX-XXXX-XXXX-XXXX"
            disabled={inputOk}
            className={`flex-1 border rounded-lg px-4 py-3 text-sm font-mono text-gray-800 placeholder:text-gray-300 focus:outline-none transition-colors disabled:bg-gray-50 disabled:cursor-not-allowed ${
              inputOk      ? "border-green-400 bg-green-50"
          ...
          <button
            ...
            disabled={inputOk || validating}
            className={`px-5 py-3 rounded-lg text-sm font-semibold transition-all shrink-0 flex items-center gap-1.5 ${
              inputOk
                ? "bg-green-500 text-white cursor-default"
                : "border border-gray-300 hover:border-gray-500 text-gray-700 hover:text-black disabled:opacity-50"
            }`}
          >
            {validating
        ...
        {inputOk && (
          <p className="text-xs text-green-600 mt-2 flex items-center gap-1.5 font-semibold">
            <CheckCircle2 size={12} /> Key validated — you may proceed to checkout.
          </p>
        )}
      </div>
      ...
      {history.length > 0 && (
        <div>
          <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-gray-400 mb-2">
            Key generation history (this session)
          </p>
          <p className="text-xs text-gray-400 mb-3 leading-relaxed">
            Every key you generate is stored in our database and logged here.
            Each is single-use — once validated or expired it cannot be reused.
          </p>
          <div className="overflow-x-auto">
            <table className="w-full text-sm" style={{ borderCollapse: "collapse" }}>
              <thead>
                <tr style={{ borderBottom: "2px solid #f3f4f6" }}>
                  {["License Key", "Generated At", "Expires At", "Status"].map((h) => (
                    <th key={h} className="text-left py-2.5 pr-6 text-[10px] font-bold uppercase tracking-widest text-gray-400">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {history.map((k, i) => {
                  const genTime = new Date(k.generatedAt).toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit", second: "2-digit" });
                  const expTime = new Date(k.expiresAt).toLocaleTimeString("en-US",   { hour: "2-digit", minute: "2-digit", second: "2-digit" });
                  const isActive = k.status === "active";
                  return (
                    <tr key={i} style={{ borderBottom: "1px solid #fafafa" }}>
                      <td className="py-3 pr-6 font-mono text-xs text-gray-700 font-semibold">
                        {k.key}
                        {isActive && (
                          <span className={`ml-2 text-[10px] font-bold tabular-nums ${urgentColor}`}>
                            ({fmt(timeLeft)})
                          </span>
                        )}
                      </td>
                      <td className="py-3 pr-6 text-xs text-gray-500 tabular-nums">{genTime}</td>
                      <td className="py-3 pr-6 text-xs text-gray-500 tabular-nums">{expTime}</td>
                      <td className="py-3 pr-6">
                        <span className={`text-[10px] font-bold px-2.5 py-1 rounded-full border ${statusBadge(k.status)}`}>
...
function LicenseInner() {
  ...
  const canCheckout   = termsAccepted && !!validatedKey;
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="max-w-2xl mx-auto px-6 md:px-12 pt-8 pb-32">
          ...
          <p className="text-base text-gray-500 leading-relaxed mb-12" style={{ maxWidth: "480px" }}>
            This page walks you through every aspect of the license — what you earn, what you pay, how to generate and validate your key, sign your agreement, and complete your purchase.
          </p>
          ...
          {/* 4 — ENVIRONMENT */}
          <SectionHead number="4" title="Environment" note="Your node's deployment environment determines which AI clients can allocate to it. Test is the default — switch to Production after activation to access live, paying clients." />
          <div className="ml-12 mb-12 space-y-4" style={{ fontFamily: "system-ui, sans-serif" }}>
            <table className="w-full text-sm overflow-x-auto" style={{ borderCollapse: "collapse" }}>
              ...
              <tbody>
                <tr>
                  <td className="py-4 pr-8 text-gray-700 font-medium text-sm">Test</td>
                  <td className="py-4 pr-8 text-gray-400 text-sm">—</td>
                  <td className="py-4 pr-8 text-gray-400 text-sm">—</td>
                  <td className="py-4 pr-8"><span className="text-xs font-semibold text-green-700 bg-green-50 px-2.5 py-1 rounded-full">Localhost</span></td>
                </tr>
              </tbody>
            </table>
            <div className="flex items-start gap-2.5 bg-amber-50 rounded-lg px-4 py-3">
              <AlertCircle size={13} className="text-amber-500 shrink-0 mt-0.5" />
              <p className="text-xs text-amber-700 leading-relaxed"><strong>Test environment active.</strong> Your node will not earn real revenue until you activate your license and switch to Production.</p>
            </div>
          </div>
          ...
          <div className="ml-12 mb-12"><SignatureUpload /></div>
